...
class FCN8sMultiViewMirrorSegmentationDepthEstimation(chainer.Chain):
    ...
    def __init__(self, n_class, num_view):
        ...
        with self.init_scope():
            ...
            self.conv1_1_depth = L.Convolution2D(
                1 * num_view + n_class, 64, 3, 1, 1, **kwargs)
    ...
    def predict_label(self, bgrs, depth_bgrs):
        ...
        h = upscore8_label[:, :, 31:31 + bgrs.shape[2], 31:31 + bgrs.shape[3]]
        score_label = h  # 1/1

        return score_label, pool3, pool4, pool5

    def predict_depth(self, depths, pool3, pool4, pool5, score_label):
        ...
        h = upscore8_depth[
            :, :,
            31:31 + depths.shape[2],
            31:31 + depths.shape[3]
        ]
        upscore8c_depth = h  # 1/1

        concatenated = F.concat((depths, score_label), axis=1)
    ...
    def __call__(self, bgrs, depths, depth_bgrs, label_gt=None, depth_gt=None):
        score_label, pool3, pool4, pool5 = self.predict_label(bgrs, depth_bgrs)
        depth_pred = self.predict_depth(
            depths, pool3, pool4, pool5, score_label)
