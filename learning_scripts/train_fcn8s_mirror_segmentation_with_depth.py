...
def main():
    ...
    if args.dataset == 'LabeledMirrorWithDepthDataset':
        dataset_train = LabeledMirrorWithDepthDataset(split='train', aug=True)
        dataset_valid = LabeledMirrorWithDepthDataset(split='test', aug=False)
    else:
    ...
    if args.model == 'FCN8sMirrorSegmentationWithDepth':
        model = FCN8sMirrorSegmentationWithDepth(n_class=n_class)
    else:
