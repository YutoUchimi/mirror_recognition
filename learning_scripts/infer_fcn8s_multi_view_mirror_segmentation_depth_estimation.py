...
from models import FCN8sMultiViewMirrorSegmentationDepthEstimation  # NOQA


class FCNMultiViewMirrorSegmentationDepthEstimation(object):
    ...
    def summarizing_process(self):
        ...
        for id in range(self.data_len):
            ...
            image_bgrs, depth_nan2zeros, depth_bgrs, label_gt, depth_gt = \
                self._transform(examples)
            pred_label, pred_depth = self._predict(
                image_bgrs, depth_nan2zeros, depth_bgrs)
    ...
    def _predict(self, image_bgrs, depths, depth_bgrs):
        ...
        if LooseVersion(chainer.__version__) < LooseVersion('2.0.0'):
            ...
            depths_variable = chainer.Variable(
                depths_data, volatile=True)
            depth_bgrs_variable = chainer.Variable(
                depth_bgrs_data, volatile=True)
            self.model(
                image_bgrs_variable, depths_variable, depth_bgrs_variable,
                None, None)
        else:
            with chainer.using_config('train', False):
                with chainer.no_backprop_mode():
                    image_bgrs_variable = chainer.Variable(image_bgrs_data)
                    depths_variable = chainer.Variable(depths_data)
                    depth_bgrs_variable = chainer.Variable(depth_bgrs_data)
                    self.model(
                        image_bgrs_variable, depths_variable,
                        depth_bgrs_variable, None, None)
...
if __name__ == '__main__':
    FCNMultiViewMirrorSegmentationDepthEstimation()
