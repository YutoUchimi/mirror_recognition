...
def transform(in_data):
    rgb_img, gt_mirror, gt_object = in_data
...
def main():
    ...
    if args.dataset == 'LabeledMirrorObjectDataset':
        dataset_train = LabeledMirrorObjectDataset(split='train', aug=True)
        dataset_valid = LabeledMirrorObjectDataset(split='test', aug=False)
    else:
    ...
    n_class_object = len(dataset_train.object_class_names)

    if args.model == 'FCN8sMirrorObjectSegmentation':
        model = FCN8sMirrorObjectSegmentation(n_class_object=n_class_object)
    else:
    ...
    trainer.extend(
        extensions.snapshot_object(
            model,
            savefun=chainer.serializers.save_npz,
            filename='max_mean_iu_object.npz'),
        trigger=chainer.training.triggers.MaxValueTrigger(
            'validation/main/mean_iu_object', save_interval))
    ...
    trainer.extend(chainer.training.extensions.PrintReport([
        ...
        'main/mean_iu_mirror',
        'main/mean_iu_object',
        'validation/main/mean_iu_mirror',
